...
- `romaji/` — Romaji-to-hiragana conversion
  ...
  - `fuzzy.rs` — Fuzzy romaji repair: QWERTY adjacency map, hypothesis generation (vowel insertion / deletion / substitution / transposition), and RomajiConverter-based validation for typo correction
...
- `core/engine/` — IMEEngine state machine (Empty → Composing → Conversion)
  ...
  - `conversion.rs` — Conversion mode handling (candidate building, multi-segment navigation, commit); `ConversionSegment` holds `reading + CandidateList` per segment; Left/Right moves between segments; Shift+Left/Right shrinks/expands the current segment by 1 kana char
  ...
  - `fuzzy.rs` — Fuzzy repair engine integration: generates corrected reading hypotheses from stranded ASCII in the composing buffer, filters through dictionary/learning cache, converts via model, and builds candidates for the Conversion state
...
- Conversion state supports multi-segment navigation (Google IME style). `self.segments: Vec<ConversionSegment>` and `self.current_segment: usize` are the source of truth — each segment holds its own `reading` and `CandidateList`. Left/Right arrows in Conversion state move `current_segment`; Shift+Left shrinks the current segment by 1 kana char (the removed char becomes the head of a new next segment); Shift+Right expands by stealing the first char of the next segment. Both resize operations call `build_segment_candidates` to re-convert each affected segment. `drain_segments()` collects all selected text, records learning, and clears segments on commit. Shift+Space in Empty state commits a literal half-width space without entering conversion.
- Fuzzy romaji repair operates at two levels when the user starts conversion (Space or Down). **ASCII-level**: detects stranded ASCII in the composing buffer (characters the romaji converter couldn't convert) and generates corrected reading hypotheses via 4 edit operations (vowel insertion, deletion, QWERTY-neighbor substitution, transposition). **Kana-level**: when romaji conversion succeeds but the reading has no dictionary/learning hits (likely a wrong-key typo), reverse-maps kana to romaji (`reverse_romaji`), applies QWERTY-neighbor substitutions, and re-converts. Kana repair candidates are only prepended when they have dictionary/learning backing; without it, no repair is attempted (avoids false positives). Both levels are configured in `[fuzzy_repair]` section of `config.toml` (`enabled`). Tab bypasses fuzzy repair (raw conversion)
