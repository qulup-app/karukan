...
- Fuzzy romaji repair (Ctrl+Shift+Space, configurable) detects stranded ASCII in the composing buffer — characters the romaji converter couldn't convert (typo signal) — and generates corrected reading hypotheses via 4 edit operations (vowel insertion, deletion, QWERTY-neighbor substitution, transposition). Valid hypotheses are filtered through the dictionary/learning cache and converted by the model, then presented as candidates in the normal Conversion UI. Configured in `[fuzzy_repair]` section of `config.toml` (`enabled`, `shortcut`)
